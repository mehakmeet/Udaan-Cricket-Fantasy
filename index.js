...
//MAIN_ for Cricket fanatsy
app.get('/player_inf',(req,res)=>{
	res.json(players);
});
...
app.post('/add-points',(req,res)=>{
	...
	const found=players.some(players=>players.pid===(newMem.id));

	if(found)
	{
		players.credits=newMem.credits
	}
	else{
		req.status(400).json({msg:'No id available'});
	}
